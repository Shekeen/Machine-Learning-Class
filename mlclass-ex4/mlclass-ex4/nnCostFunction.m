...
h_theta = sigmoid(Theta1 * X');
h_theta = [ones(1, size(h_theta, 2)); h_theta];
h_theta = sigmoid(Theta2 * h_theta);

for i = 1:m,
	y_vec = zeros(1, num_labels);
	y_vec(y(i)) = 1;
	h_theta_i = h_theta(:, i);
	J = J + ((-y_vec) * log(h_theta_i) - (1 - y_vec) * log(1 - h_theta_i));
end;
J = J / m;
...
J = J + lambda / (2 * m) * (Theta1_Reg(:)' * Theta1_Reg(:) + Theta2_Reg(:)' * Theta2_Reg(:));
